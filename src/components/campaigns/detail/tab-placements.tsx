...
const COLORS = ['#2563eb', '#16a34a', '#ea580c', '#9333ea', '#0891b2', '#d97706'];

interface TabPlacementsProps {
  data: BreakdownEntry[];
}

export function TabPlacements({ data }: TabPlacementsProps) {
  if (data.length === 0) {
    return (
      <Card>
        <CardContent className="py-8">
          <p className="text-sm text-muted-foreground text-center">Sin datos de ubicaciones disponibles</p>
        </CardContent>
      </Card>
    );
  }
